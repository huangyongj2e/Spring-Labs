 ...
 * @author Huangyong
 * @version 1.0.0
 * @Description监听 Apollo 中的spring.cloud.gateway 发生变化时，刷新内存中的路由信息
 * @createTime 2022-3-22 17:41
 */
@Component
public class GatewayPropertiesRefresher implements ApplicationContextAware, ApplicationEventPublisherAware {
    ...
    /**
     * 通过 Apollo 提供的 @ApolloConfigChangeListener 注解，声明监听 spring.cloud.gateway. 配置项的刷新。
     * @param changeEvent
     */
    ...
    private void refreshGatewayProperties(ConfigChangeEvent changeEvent) {
        ...
        // <1>调用 #preDestroyGatewayProperties(ConfigChangeEvent changeEvent) 方法，处理 spring.cloud.gateway.routes 或 spring.cloud.gateway.default-filters 配置项可能被删除光的特殊骚操作
        preDestroyGatewayProperties(changeEvent);
        // <2>发布 Spring Cloud Context EnvironmentChangeEvent 事件，通知 spring.cloud.gateway 配置项发生变化，从而实现使用到注入新的GatewayProperties Bean。例如说，我们这里 @Autowired 注入的 GatewayProperties。
        this.applicationContext.publishEvent(new EnvironmentChangeEvent(changeEvent.changedKeys()));
        // <3>调用 #refreshGatewayRouteDefinition() 方法，发布 Gateway RefreshRoutesEvent 事件，从而刷新内存中的路由信息。代码如下：
     ...
     * 因为 GatewayProperties 没有 @PreDestroy 注解的 destroy 方法，所以 Spring Cloud Context 的 ConfigurationPropertiesRebinder#rebind(String name) 中 destroyBean 无法销毁当前的 GatewayProperties Bean 对象。
     *
     * 如果把 spring.cloud.gateway. 前缀的配置项全部删除（例如需要动态删除最后一个路由的场景），initializeBean 时也无法创建新的 GatewayProperties Bean 对象，则 return 当前的 GatewayProperties Bean 对象。
     * 若仍保留有 spring.cloud.gateway.routes[n] 或 spring.cloud.gateway.default-filters[n] 等配置项，initializeBean 会时会注入新的 GatewayProperties Bean 对象。
